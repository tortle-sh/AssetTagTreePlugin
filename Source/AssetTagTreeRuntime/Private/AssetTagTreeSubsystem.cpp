...
TArray<TSoftObjectPtr<UObject>> UAssetTagTreeSubsystem::FindObjects(
...
void UAssetTagTreeSubsystem::RegisterCallbackOnNodes(
	const FCallbackDelegate& CallbackDelegate,
	const FGameplayTagContainer& Tags)
{
...
void UAssetTagTreeSubsystem::RemoveCallbackFromNodes(
	const FCallbackDelegate& CallbackDelegate,
	const FGameplayTagContainer& Tags)
{
	const FGameplayTagContainer FilteredTags = FilterTags(Tags);
                                                ...
                                                const FGuid& SubjectId)
{
	...
	for (const auto Node : RootNodes)
	{
		Node->InsertToTags(InsertedObject, FilteredTags, SubjectId);
	}
}

void UAssetTagTreeSubsystem::CollectNodeHashes(
	TMap<FGameplayTag, uint32>& NodeHashCollection,
	const FGameplayTagContainer& Tags,
	const int32 TagCollectionFlags) const
{
	FGameplayTagContainer RelevantTags;

	CollectTags(Tags, TagCollectionFlags, RelevantTags);
	TSet<UAssetTagTreeNode*> RelevantNodes;

	for (const auto Node : RootNodes)
	{
		RelevantNodes.Append(Node->FindAllNodesByTags(RelevantTags));
	}
	
	for (const auto Node : RelevantNodes)
	{
		NodeHashCollection.Emplace(Node->GetTag(), Node->GetNodeHash());
	}
}
                                                  ...
                                                  const FGuid& SubjectId)
{
	...
	for (const auto Node : RootNodes)
	{
		Node->RemoveObjectFromTags(RemovedObject, FilteredTags, SubjectId);
	}
}

void UAssetTagTreeSubsystem::CollectTags(const FGameplayTagContainer& TargetTags, const int32 TagCollectionFlags,
